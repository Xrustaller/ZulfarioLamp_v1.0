...
void drawFrame(int pcnt)
{
		...
		for (unsigned char x = 0; x < WIDTH; x++)
		{
			...
			if (y < 8)
			{
				...
				CRGB color = CHSV(
					...
					255, // S
					static_cast<uint8_t>(max(0, nextv)) // V
				);
	...
	for (unsigned char x = 0; x < WIDTH; x++)
	{
		...
		CRGB color = CHSV(
			...
			255, // S
			static_cast<uint8_t>(((100.0 - pcnt) * matrixValue[0][newX] + pcnt * line[newX]) / 100.0) // V
		);
...
void rainbowVertical()
{
	...
	for (byte j = 0; j < HEIGHT; j++)
	{
		CHSV thisColor = CHSV(static_cast<byte>(hue + j * modes[2].scale), 255, 255);
		for (byte i = 0; i < WIDTH; i++)
...
void rainbowHorizontal()
{
	...
	for (byte i = 0; i < WIDTH; i++)
	{
		CHSV thisColor = CHSV(static_cast<byte>(hue + i * modes[3].scale), 255, 255);
		for (byte j = 0; j < HEIGHT; j++)
...
void whiteLamp()
{
	for (byte y = 0; y < (HEIGHT / 2); y++)
	{
		CHSV color = CHSV(
			100, 1,
			constrain(modes[17].brightness - (long)modes[17].speed * modes[17].brightness / 255 * y / 2, 1, 255));
		for (byte x = 0; x < WIDTH; x++)
		{
